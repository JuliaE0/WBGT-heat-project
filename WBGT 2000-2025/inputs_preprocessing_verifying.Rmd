...
for (file in files) {
  ...
  nc_temp <- nc_create("temp.nc", list(v_cza, v_fdir, v_solarRet))
  ...
  input_file = '", file, "'
  temp_file  = 'temp.nc'
  output_file = Path('wbgt_inputs_complete') / Path(input_file).name
...
```{r}
# open the preprocessed dataset created in part 1

# open and view preprocessed dataset -------- LOOP OVER ALL FILES. NEED TO WORK ON
inputs_p1 <- nc_open("wbgt_inputs_part1/wbgt_inputs_{year}_{month}.nc", write = TRUE) 
# something like this

#names(wbgt_inputs_aug_2019$var)
#dim(ncvar_get(wbgt_inputs_aug_2019, "fdir_frac")) # R shows dimensions as longitude x latitude x times
#ncatt_get(wbgt_inputs_aug_2019, "valid_time", "units")
```
...
LOOP EVERYTHING BELOW OVER MONTHS -- save into a folder called wbgt_inputs_complete
...
dim(ncvar_get(wbgt_inputs_aug_2019, "fdir_frac"))
...
year  <- ncvar_get(wbgt_inputs_aug_2019, "year")
month <- ncvar_get(wbgt_inputs_aug_2019, "month")
day   <- ncvar_get(wbgt_inputs_aug_2019, "dday")
solar <- ncvar_get(wbgt_inputs_aug_2019, "ssrd_W")  # ERA5 ssrd in W/m2
cza <- v_cza_array  # calculated from calc_cza_int()
fdir <- ncvar_get(wbgt_inputs_aug_2019, "fdir_frac") # (NN-inter ERA5 fdir)/(NN-inter ERA5 ssrd)
...
dim(ncvar_get(wbgt_inputs_aug_2019, "ssrd_W"))
...
```{r}
# view temp2.nc
temp <- nc_open("temp.nc")
names(temp$var)
dim(ncvar_get(temp, "v_fdir"))
nc_close(temp)
```
...
ds = xr.open_dataset("wbgt_inputs_aug_2019.nc")
