...
```{r}
# check that dimensions of cza_array match dimensions of ds_work
dim(ncvar_get(ds_work, "solar"))
...
```{r}
# put cza_array_ca into ds_work as a variable -- edited ds_work directly

nc_close(ds_work)

ds_work <- nc_open("ds_work.nc", write = TRUE)

dim_lon  <- ds_work$dim$longitude
dim_lat  <- ds_work$dim$latitude
dim_time <- ds_work$dim$valid_time
...
ds_work <- ncvar_add(ds_work, cza)

ncvar_put(ds_work, cza, cza_array_ca)
...
```{r}
# just checking
# plot with ds_work Tair variable at a single valid_time

r <- rast("ds_work.nc", subds = "Tair")
...
plot(r[[t_index]])
title(main = "Tair at 2019-08-01 12:00", line=3)
```
